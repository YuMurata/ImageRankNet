...
def make_example(image1: np.array, image2: np.array, label: int):
    ...
    feature = tf.train.Features(feature={
        'image1': tf.train.Feature(bytes_list=tf.train.BytesList(value=[image1.tobytes()])),
        'image2': tf.train.Feature(bytes_list=tf.train.BytesList(value=[image2.tobytes()])),
...
if __name__ == "__main__":
    dataset_dir = Path(__file__).parent/'tfrecords'
    write_tfrecord(str(dataset_dir/'train.tfrecord'))
    write_tfrecord(str(dataset_dir/'validation.tfrecord'))
